...
public class Twig : MonoBehaviour
{
    ...
    {
        Vector3 direction = (_target.position - transform.position).normalized; //플레이어와 이펙트 프리펩과의 방향

        Vector3 rotationDir = Quaternion.LookRotation(direction).eulerAngles;

        CheckDiretion(rotationDir);
    ...
    private void CheckDiretion(Vector3 _rotationDir)
    {
